...
- Pi, provider authentication, extension UI, and trusted-provenance non-core adapters stay on the host. Except for explicit `--yolo` launches, every model-directed core file and shell operation runs through the per-operation SRT controller.
