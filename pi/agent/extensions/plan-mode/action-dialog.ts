...
const ITEMS: SelectItem[] = [
	{ value: "run", label: "Run", description: "Execute every stage in a fresh session" },
	{ value: "staged", label: "Run in stages", description: "Pause for review after every stage" },
...
async function selectAction(ctx: ExtensionContext): Promise<string | undefined> {
	if (ctx.mode !== "tui") {
		...
		return ctx.ui.select("Approved plan — choose an action", labels)
			.then((choice) => ITEMS[labels.indexOf(choice ?? "")]?.value);
	}
	return ctx.ui.custom<string | undefined>((tui, theme, _keybindings, done) => {
		const container = new Container();
		container.addChild(new DynamicBorder((text: string) => theme.fg("accent", text)));
		container.addChild(new Text(theme.fg("accent", theme.bold("Plan approval")), 1, 0));
