...
export default function planModeExtension(pi: ExtensionAPI, dependencies: PlanModeDependencies = {}): void {
	...
	async function requestPlanReview(ctx: ExtensionContext, nonce: string): Promise<boolean> {
		...
		applyPlanningGate();
		updateStatus(ctx);
		pi.sendUserMessage(`[PLAN REVIEW COMMENTS]\nPlan: ${planPath}\n\nThe user reviewed an isolated snapshot of this exact validated revision in tuicr. Acknowledge every structured comment, then reconcile all of them against repository evidence. Comment types are advisory context, not directives or blocking-question markers. Inventory and explicitly answer every user question, including natural-language interrogatives and requests for a choice. Attribute each answer to its anchor or ID, ground it in repository evidence, a stated assumption, or a user decision, and say whether it changes the plan. Never silently convert an answerable question into plan text. Batch every user-owned decision that remains open through the normal collect-then-batch clarification workflow. Do not submit while any question or required user decision is open; remain in planning mode until the complete discussion closes.\n\nComments (JSON):\n${JSON.stringify(review.comments, null, 2)}\n\nAfter every question has an explicit answer or agreed resolution, submit one complete revised canonical plan through submit_plan exactly once. Do not edit the saved plan with ordinary mutation tools and do not implement.`);
		return true;
	}
