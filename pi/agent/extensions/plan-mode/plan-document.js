...
export function splitManagedProgressReport(markdown) {
	...
	if (
		reportLines[1] !== PROGRESS_REPORT_HEADING || reportLines[2] !== "" ||
	...
	if (rowLines.length === 0 || rowLines.some((line) => !/^- (?:☐|▶|☑|⛔) .+$/.test(line))) {
		throw new ManagedProgressReportError("malformed_progress_report", "Managed progress report rows must contain a status icon and Step title");
	}
	return {
		coreMarkdown: lines.slice(0, start).join("\n").trimEnd(),
		report: { rows: rowLines.map((line) => line.slice(2)) },
	};
}

export function renderManagedProgressReport(rows) {
	if (!Array.isArray(rows) || rows.length === 0) {
		throw new ManagedProgressReportError("malformed_progress_report", "Managed progress report requires at least one Step row");
	}
	return [
...
export function replaceManagedProgressReport(markdown, rows, options = {}) {
	...
	const rendered = `${coreMarkdown}\n\n${renderManagedProgressReport(rows)}\n`;
...
export function parsePlanDocument(markdown, options = {}) {
	...
	if (result.ok && split.report) result.document.managedProgressReport = true;
	return result;
}
...
export function renderPlanDocument(document) {
	...
	else rendered = renderVersion3Document(document);
	return document.managedProgressReport
		? replaceManagedProgressReport(rendered, buildDocumentStepProgressRows(document))
		: rendered;
}
