...
function createHarness(t, options = {}) {
  ...
  if (options.root) {
    for (const name of ["PI_GONDOLIN_SOCKET", "PI_GONDOLIN_LEASE", "PI_GONDOLIN_ROOT_OWNER_PID", "PI_GONDOLIN_WORKSPACE_KEY", "PI_GONDOLIN_WORKSPACE_ROOT", "PI_GONDOLIN_POLICY_GENERATION", "PI_GONDOLIN_IMAGE_GENERATION", "PI_GONDOLIN_VM_ID"]) delete env[name];
...
test("root replacements retain one lease and VM for every Pi replacement reason", async (t) => {
  for (const reason of ["new", "resume", "fork", "reload"]) {
    const shared = { PI_GONDOLIN_SANDBOX: "1", PI_GONDOLIN_BUILTIN_TOOLS: "read,bash", PI_GONDOLIN_HOST_TOOLS: "" };
    ...
    const vmId = shared.PI_GONDOLIN_VM_ID;
    await first.emit("session_shutdown", { reason });
    assert.equal(firstClient.releaseCalls, 0, `${reason} must not release the root lease`);
    assert.equal(shared.PI_GONDOLIN_LEASE, HEX_A, `${reason} must retain the capability`);
    assert.equal(shared.PI_GONDOLIN_VM_ID, vmId, `${reason} must retain the VM identity`);
    ...
    assert.equal(shared.PI_GONDOLIN_LEASE, undefined);
    assert.equal(shared.PI_GONDOLIN_ROOT_OWNER_PID, undefined);
  }
});
...
test("pending root replacement cancels only the old waiter and leaves cold startup reusable", async (t) => {
  ...
  assert.ok(shared.PI_GONDOLIN_STARTUP_DESCRIPTOR, "replacement keeps the original startup descriptor");
  assert.equal(shared.PI_GONDOLIN_LEASE, undefined);
  await new Promise((resolve) => setImmediate(resolve));
  ...
  assert.deepEqual(second.active().sort(), ["bash", "read"]);
  assert.equal(shared.PI_GONDOLIN_ROOT_OWNER_PID, String(process.pid));
  await second.emit("session_shutdown", { reason: "quit" });
...
test("child replacements reconnect but never adopt or release the parent root lease", async (t) => {
  ...
  const child = createHarness(t, {
    client: firstClient,
    env: {
      PI_GONDOLIN_SANDBOX: "1", PI_GONDOLIN_SOCKET: "/tmp/controller.sock", PI_GONDOLIN_LEASE: HEX_A,
      PI_GONDOLIN_ROOT_OWNER_PID: ownerPid, PI_GONDOLIN_WORKSPACE_KEY: HEX_C,
      PI_GONDOLIN_WORKSPACE_ROOT: "/physical/workspace", PI_GONDOLIN_POLICY_GENERATION: HEX_B,
      PI_GONDOLIN_IMAGE_GENERATION: HEX_A, PI_GONDOLIN_VM_ID: "vm-shared",
      PI_GONDOLIN_BUILTIN_TOOLS: "read,bash", PI_GONDOLIN_HOST_TOOLS: "",
    },
  });
  ...
  assert.equal(firstClient.releaseCalls, 0);
  assert.equal(child.env.PI_GONDOLIN_ROOT_OWNER_PID, ownerPid);
...
test("retired status callbacks cannot change the replacement lifecycle", async (t) => {
  ...
  const lifecycle = [];
  harness.pi.events.on("gondolin-sandbox:lifecycle", (event) => lifecycle.push(event.health));
  await harness.emit("session_start", { reason: "startup" });
...
test("fatal root routing failures release the active lease before clearing capabilities", async (t) => {
  ...
  const harness = createHarness(t, {
    ...
    acquire: async ({ startup }) => ({
      ...
      scope: { workspaceKey: HEX_C, canonicalWorkspaceRoot: "/physical/workspace" },
      status: { health: "healthy", dockerHealthy: false, vmId: "vm-shared", workspaceKey: HEX_C,
        workspaceRoot: "/physical/workspace", policyGeneration: HEX_B, imageGeneration: HEX_A, attachedRoots: 1 },
    }),
  });
  await harness.emit("session_start", { reason: "startup" });
  ...
  assert.equal(harness.env.PI_GONDOLIN_LEASE, undefined);
});
