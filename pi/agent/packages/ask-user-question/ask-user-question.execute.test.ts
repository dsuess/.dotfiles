...
describe("ask_user_question.execute — ctx.ui.custom dispatch", () => {
	...
	it("handoff settles as non-decline and queues exactly one immediate normal-chat continuation", async () => {
		const { pi, captured } = createMockPi();
		registerAskUserQuestionTool(pi);
		const tool = captured.tools.get("ask_user_question")!;
		const sendUserMessage = vi.mocked(pi.sendUserMessage);
		const ctx = ctxWithCustom({
			cancelled: false,
			outcome: "handoff",
			answers: [],
			handoff: {
				...
				reason: "Choices are incomplete",
				transcript: [{ role: "user", text: "I need a third path" }],
				partialAnswers: [],
			},
		});
		const r = await tool.execute?.("tc", BASE_PARAMS as never, undefined as never, undefined as never, ctx as never);
		expect(r?.details).toMatchObject({ cancelled: false, outcome: "handoff" });
		expect(r?.content[0]).toMatchObject({ text: expect.stringContaining("continue this clarification") });
		expect(r?.content[0]).toMatchObject({ text: expect.not.stringContaining("declined") });
		expect(r?.terminate).toBe(true);
		expect(sendUserMessage).toHaveBeenCalledTimes(1);
		expect(sendUserMessage).toHaveBeenCalledWith(
			expect.stringContaining("[Questionnaire handoff]"),
			{ deliverAs: "steer" },
		);
		expect(sendUserMessage.mock.calls[0]![0]).toContain("Choices are incomplete");
		expect(sendUserMessage.mock.calls[0]![0]).toContain("I need a third path");
	});
