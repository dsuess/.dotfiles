...
describe("ask_user_question — single-question navigation", () => {
	...
	it("UP from Alpha wraps to Type-something, then passes Discuss to Gamma", async () => {
		const tool = register();
		const { custom } = driveCustom((c) => {
			// Items = [Alpha, Beta, Gamma, Discuss, Type something].
			c.handleInput(KEY.UP); // Alpha → wrap to Type something
			c.handleInput(KEY.UP); // Type something → Discuss
			c.handleInput(KEY.UP); // Discuss → Gamma
...
describe("ask_user_question — 'Type something.' free-text flow", () => {
	...
	it("navigate to Other sentinel, type text, Enter → kind:'custom' with typed text", async () => {
		const tool = register();
		const { custom } = driveCustom((c) => {
			// Items: [Default, Second, Discuss, Type something.]
			c.handleInput(KEY.DOWN); // → Second
			c.handleInput(KEY.DOWN); // → Discuss
...
describe("ask_user_question — multi-select flow (single question)", () => {
	...
	it("Space toggles items, then DOWN to Next + Enter confirms selected labels", async () => {
		...
		const { custom } = driveCustom((c) => {
			...
			c.handleInput(KEY.DOWN); // → Discuss this
...
describe("ask_user_question — mixed single+multi question flow", () => {
	it("answer single-select Q1, toggle multi-select Q2, submit", async () => {
		...
		const { custom } = driveCustom((c) => {
			...
			c.handleInput(KEY.DOWN); // → Discuss this (5)
			c.handleInput(KEY.DOWN); // → "Type something." row (6)
			c.handleInput(KEY.DOWN); // → Next sentinel (7)
...
describe("ask_user_question — multi-select notes end-to-end", () => {
	it("n → type → Esc → Space → Next+Enter lands a multi answer carrying the typed note", async () => {
		const tool = register();
		const { custom } = driveCustom((c) => {
			// Items: [FE (0), BE (1), Discuss (2), Type something (3), Next (4)]. Startup focuses FE.
