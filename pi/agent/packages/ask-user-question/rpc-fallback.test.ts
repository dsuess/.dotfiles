...
import { emptyDiscussionUsage } from "./discussion/types.js";
...
describe("ask_user_question.execute — RPC dialog walker (ctx.mode === 'rpc')", () => {
	...
	it("appends Discuss this before the custom row", async () => {
		const tool = register();
		let offered: string[] = [];
		...
		await run(tool, SINGLE, ctxRpc({ select }));
		expect(offered).toEqual(["1. A — a", "2. B — b", "3. Discuss this", "4. Type something."]);
	});
	...
	it("multi-select uses ctx.ui.input and parses comma-separated indices into labels", async () => {
		...
		const r = await run(tool, MULTI, ctxRpc({ select, input }));
	...
	it("walks multiple questions sequentially, one dialog each", async () => {
		...
		const select = vi.fn(async (_t: string, options: string[]) => {
			call++;
			return call === 1 ? options[0] : options[options.length - 1];
		});
...
describe("native-dialog discussion lifecycle", () => {
	it("supports repeated turns, shows prior responses, returns, and answers with discussion metadata", async () => {
		const selections = [2, 0, 0, 1, 0];
		let selection = 0;
		const titles: string[] = [];
		const ui = {
			select: vi.fn(async (title: string, options: string[]) => {
				titles.push(title);
				return options[selections[selection++]!];
			}),
			input: vi.fn()
				.mockResolvedValueOnce("Why A?")
				.mockResolvedValueOnce("And long term?"),
		};
		const runner = vi.fn(async ({ userPrompt }: { userPrompt: string }) => ({
			response: `Answer to ${userPrompt}`,
			usage: emptyDiscussionUsage(),
			truncated: false,
			tools: ["read"],
		}));
		const result = await runRpcQuestionnaire(ui, SINGLE, runner as never);
		expect(runner).toHaveBeenCalledTimes(2);
		expect(runner.mock.calls[1]![0]).toMatchObject({
			transcript: [
				{ role: "user", text: "Why A?" },
				{ role: "assistant", text: "Answer to Why A?" },
			],
		});
		expect(titles.some((title) => title.includes("Answer to Why A?"))).toBe(true);
		expect(result).toMatchObject({
			cancelled: false,
			answers: [expect.objectContaining({ answer: "A" })],
			discussions: [expect.objectContaining({ question: "Which?" })],
		});
	});

	it("returns a non-cancelled handoff with partial answers", async () => {
		const params = { questions: [...SINGLE.questions, { ...SINGLE.questions[0]!, question: "Second?" }] };
		const selections = [0, 2, 2]; // answer first; Discuss second; Continue in chat
		let selection = 0;
		const ui = {
			select: vi.fn(async (_title: string, options: string[]) => options[selections[selection++]!]),
			input: vi.fn(async () => ""),
		};
		...
		expect(result).toMatchObject({
			...
			handoff: {
				question: "Second?",
				partialAnswers: [expect.objectContaining({ answer: "A" })],
			},
		});
	});
});
