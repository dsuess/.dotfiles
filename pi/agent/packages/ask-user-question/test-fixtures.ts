...
export function makeMultiSelectViewProps(
	...
	const discussActive = focused && optionIndex === question.options.length;
	const otherActive = focused && optionIndex === question.options.length + 1;
	const nextActive = focused && optionIndex === question.options.length + 2;
...
export function makeMultiSelectPropsFromState(
	...
	return {
		rows,
		discussActive: focused && state.optionIndex === question.options.length,
		discussLabel: "Discuss this",
		other: {
			active: focused && state.optionIndex === question.options.length + 1,
