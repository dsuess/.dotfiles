...
export const selectMultiSelectProps: PerTabSelector<MultiSelectViewProps> = (state, ctx) => {
	...
	const discussActive = focused && state.optionIndex === question.options.length;
	const otherActive = focused && state.optionIndex === question.options.length + 1;
	const nextActive = focused && state.optionIndex === question.options.length + 2;
