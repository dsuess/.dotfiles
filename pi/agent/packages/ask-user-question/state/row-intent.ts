...
export const SENTINEL_KINDS: readonly SentinelKind[] = ["discuss", "other", "next"];
 ...
 *   on this row is suppressed. Discussion and Next are true.
 ...
 *   option carries a `preview`. Discussion and custom input are true.
 * - `autoAppendOnMultiSelect` — `buildItemsForQuestion` appends this row
 *   when the question is multi-select. Discussion, custom input, and Next are true.
 */
...
export const LABELS_BY_KIND: { readonly [K in SentinelKind]: string } = {
	discuss: ROW_INTENT_META.discuss.label,
