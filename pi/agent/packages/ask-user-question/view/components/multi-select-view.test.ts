...
describe("MultiSelectView.render", () => {
	it("renders options, Discuss this, Type something, and a trailing Next sentinel", () => {
		const q = question();
		const m = makeView(q, makeProps(q));
		const lines = m.render(80);
		expect(lines.length).toBe(6); // 3 options + Discuss + Type something. + Next
		expect(lines[0]).toContain("FE");
		expect(lines[1]).toContain("BE");
		expect(lines[2]).toContain("DB");
		expect(lines[3]).toContain("Discuss this");
		expect(lines[4]).toContain("Type something.");
	...
	it("renders description on continuation line when present", () => {
		...
		const m = makeView(q, makeProps(q));
		const lines = m.render(80);
		expect(lines.length).toBe(6); // FE + description + BE + Discuss + custom + Next
...
describe("MultiSelectView — 'Type something.' row", () => {
	...
	const otherActiveInput = (q: QuestionData, buffer: string): MultiSelectViewProps =>
		makeProps(q, {
			optionIndex: q.options.length + 1,
			inputMode: true,
			inputBuffer: buffer,
		});

	it("renders the other row after discussion, numbered N+2, box always [ ] muted", () => {
		const q = question();
		const m = makeView(q, otherInactive(q));
		const lines = m.render(80);
		// 3 options → Discuss is row 3 and custom input is row 4, numbered "5.".
		const raw = lines[4].replace(/\x1b\[[0-9;]*m/g, "");
		expect(raw).toMatch(/^ {2}5\. \[ \] Type something\./);
	...
	it("focusedItemRowRange covers the other row ([row, row+1]) then Next", () => {
		...
		const [start, end] = view.focusedItemRowRange(80);
		expect(start).toBe(3); // after 2 options and Discuss this
		expect(end).toBe(4);
	});

	it("nextActive shifts below discussion and the custom row", () => {
		const q: QuestionData = {
			question: "pick?",
			header: "H",
			options: [{ label: "A", description: "" }],
		...
		const [start, end] = view.focusedItemRowRange(80);
		// option(0) + discuss(1) + other(2) → Next is at row 3.
		expect(start).toBe(3);
		expect(end).toBe(4);
	});

	it("number column fits N+2 (the custom row's number) without truncation", () => {
		// 9 options → Discuss is #10 and custom is #11; numberWidth must remain 2.
		const q = question({
			options: Array.from({ length: 9 }, (_, i) => ({ label: `o${i + 1}`, description: "" })),
		});
		const m = makeView(q, makeProps(q));
		const lines = m.render(80);
		const raw = lines[10].replace(/\x1b\[[0-9;]*m/g, ""); // custom row
		expect(raw).toMatch(/11\. \[ \] Type something\./);
	});
});
