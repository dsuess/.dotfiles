...
export interface MultiSelectOtherRowProps {
	/** The "Type something." row is focused after the extension-owned discussion row. */
	active: boolean;
...
export class MultiSelectView implements StatefulView<MultiSelectViewProps> {
	...
	constructor(
		...
		this.props = {
			rows: [],
			discussActive: false,
			discussLabel: displayLabel("discuss"),
	...
	private layout(width: number): MultiSelectLayout {
		...
		const discussStart = lines.length;
		const discussPointer = this.props.discussActive ? this.theme.fg("accent", ACTIVE_POINTER) : INACTIVE_POINTER;
		const discussNumber = String(this.question.options.length + 1).padStart(numberWidth, " ");
		const canonicalDiscussLabel = this.props.discussLabel ?? displayLabel("discuss");
		const discussLabel = this.props.discussActive
			? this.theme.fg("accent", this.theme.bold(canonicalDiscussLabel))
			: canonicalDiscussLabel;
		lines.push(
			truncateToWidth(`${discussPointer}${discussNumber}${NUMBER_SEPARATOR}${discussLabel}`, width, ""),
		);
		if (this.props.discussActive) focusedRange = [discussStart, lines.length];
	...
	private prefixVisibleWidth(): number {
		...
		// The number column fits `options.length + 2` so the Discuss and custom rows
		// are never clipped. The Next sentinel uses a bare `pointer + "Next"` shape — its width
