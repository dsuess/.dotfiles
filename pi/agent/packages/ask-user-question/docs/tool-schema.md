...
Authoring any of `"Other"`, `"Discuss this"`, `"Type something."`, or `"Next"` as an
option label is rejected with `reserved_label`. The last three are extension-owned runtime
rows; `"Other"` is reserved because models are conditioned to reach for it.
...
```ts
{
  ...
  details: {
    ...
    cancelled: boolean,
    outcome?: "handoff",          // present for Continue in chat
    discussions?: Array<{
      questionIndex: number,
      question: string,
    ...
    discussionUsage?: Usage,       // aggregate nested child usage/cost
    handoff?: {
      questionIndex: number,
      question: string,
  ...
  usage?: Usage,                   // same aggregate for Pi session accounting
  terminate?: true,                // handoff only
}
```
...
appended as `selected preview: <markdown>`, a note as `user notes: <text>`, and bounded
clarification material as additive discussion context.

Cancelling, and any result that produces no answer segments, retain the canonical string
`User declined to answer questions`. Partial submission remains allowed.

**Continue in chat is different:** it returns `cancelled: false`, `outcome: "handoff"`,
and text beginning `User chose to continue this clarification in normal chat.` The tool
queues exactly one steering user message with the question, authored choices, bounded
transcript, reason, and partial answers, then sets `terminate: true` so the parent receives
one coherent continuation rather than decline handling or duplicate automatic responses.

Discussion transcript/output are bounded and truncation is marked. Nested usage is present
both in `details.discussionUsage` and the tool result's top-level `usage`, so Pi includes it
in normal session totals.

## Event contract

The package publishes a prompt event after validation and a blocked-state event around
TUI/RPC user interaction. Import them from the `/events` subpath:
...
} from "@dsuess/pi-ask-user-question/events";
...
The channels are `rpiv:ask-user:prompt` and `rpiv:ask-user:blocked`. The latter emits
`{ active: true }` while input is awaited and `{ active: false }` in cleanup. Preview
*content* is deliberately not shipped
