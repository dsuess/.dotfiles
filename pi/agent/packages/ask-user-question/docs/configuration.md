...
Terminal discussion children receive internal environment markers and a private parent-system-prompt file. These values are not user settings. The child uses Pi's active model and credential resolution. Its built-ins use the parent SRT tool routing controller. Native built-ins remain disabled. Parent session identity variables are removed before the child starts.
