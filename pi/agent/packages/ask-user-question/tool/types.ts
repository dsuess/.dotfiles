 ...
 * of these labels triggers the `reserved_label` runtime guard. Three labels
 * come from `ROW_INTENT_META` (the runtime kinds); `"Other"` is
...
export const QuestionSchema = Type.Object({
	...
	options: Type.Array(OptionSchema, {
		...
		description:
			"The authored choices for this question. Must have 2-4 options. Each option should be distinct and mutually exclusive unless multiSelect is enabled. 'Discuss this' and 'Type something.' are appended automatically — do NOT author them.",
	}),
...
export type QuestionnaireOutcome = "answered" | "cancelled" | "handoff";

export interface QuestionnaireResult {
	answers: QuestionAnswer[];
	cancelled: boolean;
	/** Additive lifecycle discriminator. Legacy consumers may continue reading `cancelled`. */
	outcome?: QuestionnaireOutcome;
	discussions?: QuestionDiscussionContext[];
	discussionUsage?: DiscussionUsage;
	handoff?: QuestionnaireHandoff;
